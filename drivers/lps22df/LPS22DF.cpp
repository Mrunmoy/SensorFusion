...
bool LPS22DF::init() {
    ...
    // Software reset
    if (!bus_.write8(addr, CTRL_REG2, 0x04)) return false;
    delay_.delayMs(10);
    ...
    if (!bus_.write8(addr, CTRL_REG2, 0x08)) return false;
...
bool LPS22DF::readPressure(float& hPa) {
    uint8_t buf[3];
    if (!bus_.readRegister(cfg_.address, PRESS_OUT_XL, buf, 3)) return false;
...
bool LPS22DF::readAll(float& hPa, float& tempC) {
    ...
    uint8_t buf[5];
    if (!bus_.readRegister(cfg_.address, PRESS_OUT_XL, buf, 5)) return false;
...
bool LPS22DF::enableDataReadyInterrupt(IGpioInterrupt* intPin,
    ...
    if (!bus_.write8(addr, CTRL_REG4, 0x18)) return false;
...
bool LPS22DF::disableDataReadyInterrupt() {
    ...
    // Clear CTRL_REG4
    if (!bus_.write8(addr, CTRL_REG4, 0x00)) return false;
