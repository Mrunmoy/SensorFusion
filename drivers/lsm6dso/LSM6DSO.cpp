...
bool LSM6DSO::init() {
    ...
    if (!bus_.write8(addr, CTRL3_C, 0x01)) return false;
    delay_.delayMs(10);
...
bool LSM6DSO::readAccel(AccelData& out) {
    uint8_t buf[6];
    if (!bus_.readRegister(cfg_.address, OUTX_L_A, buf, 6)) return false;
...
bool LSM6DSO::readGyro(GyroData& out) {
    uint8_t buf[6];
    if (!bus_.readRegister(cfg_.address, OUTX_L_G, buf, 6)) return false;
...
bool LSM6DSO::readTemperature(float& tempC) {
    uint8_t buf[2];
    if (!bus_.readRegister(cfg_.address, OUT_TEMP_L, buf, 2)) return false;
...
bool LSM6DSO::readAll(AccelData& accel, GyroData& gyro, float& tempC) {
    // Read 14 contiguous bytes starting at OUT_TEMP_L (0x20):
    // [0:1] temp, [2:7] gyro XYZ, [8:13] accel XYZ
    uint8_t buf[14];
    if (!bus_.readRegister(cfg_.address, OUT_TEMP_L, buf, 14)) return false;
...
bool LSM6DSO::enableDataReadyInterrupt(IGpioInterrupt* intPin,
    ...
    if (!bus_.write8(addr, INT1_CTRL, 0x03)) return false;
...
bool LSM6DSO::disableDataReadyInterrupt() {
    ...
    if (!bus_.write8(addr, INT1_CTRL, 0x00)) return false;
